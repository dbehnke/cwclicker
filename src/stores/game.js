...
export const useGameStore = defineStore("game", () => {
	...
	function checkLotteryTrigger() {
		...
		const totalFactories = Object.values(factoryCounts.value).reduce(
			(sum, count) => sum + count,
			0,
		);
		if (totalFactories === 0) {
	...
	function triggerLottery() {
		...
		// Pick random factory
		const randomFactoryId =
			ownedFactoryIds[Math.floor(Math.random() * ownedFactoryIds.length)];

		// Pick random phenomenon title
		const randomTitle =
			PHENOMENA_TITLES[Math.floor(Math.random() * PHENOMENA_TITLES.length)];
	...
	function getLotteryMultiplier(factoryId) {
		...
		if (
			lotteryState.value.isSolarStorm &&
			now < lotteryState.value.solarStormEndTime
		) {
	...
	function revealAffordableFactories() {
		...
		const revealPool = FACTORIES.filter(
			(factory) => factory.tier <= maxUnlockedTier.value,
		);
		let changed = false;
		...
		while (true) {
			const nextFactory = revealPool.find(
				(factory) => !nextRevealed.has(factory.id),
			);
			if (!nextFactory) {
				break;
			}
	...
	function classifyMorseTapDuration(durationMs) {
		...
		const ditBase = Math.max(
			MORSE_MIN_PRESS_MS,
			Math.min(quantileDit, MORSE_TIMING.DAH_MIN_MS * 2),
		);
		...
		return Math.abs(safeDuration - ditTarget) <=
			Math.abs(safeDuration - dahTarget)
			? "dit"
			: "dah";
	}
	...
	const eligiblePrestigeLevel = computed(() => {
		...
		cachedEligiblePrestigeLevel = cubeRootFloor(
			earned / PRESTIGE_QSOS_PER_LEVEL,
		);
		cachedEligiblePrestigeThreshold = prestigeThresholdForLevel(
	...
	const canPrestigeReset = computed(
		() => eligiblePrestigeLevel.value > prestigeLevel.value,
	);
	...
	function prestigeReset() {
		...
		const newPoints =
			eligibleLevel > prestigeLevel.value
				? eligibleLevel - prestigeLevel.value
				: 0n;

		prestigePoints.value += newPoints;
		prestigeLevel.value =
			eligibleLevel > prestigeLevel.value ? eligibleLevel : prestigeLevel.value;
	...
	function getFactoryCost(factoryId, owned) {
		...
		const cost = factory.baseCost * multiplier ** owned;
	...
	function buyFactory(factoryId, count = 1) {
		...
		qsos.value -= cost;
		factoryCounts.value[factoryId] =
			(factoryCounts.value[factoryId] || 0) + purchaseCount;
		...
		try {
			const state = {
				...
				factoryProductionTotals: Object.fromEntries(
					Object.entries(factoryProductionTotals.value).map(
						([factoryId, total]) => [factoryId, total.toString()],
					),
				),
	...
	function load() {
		try {
			...
			if (saved) {
				...
				if (!state.version) {
					console.warn(
						`Detected v1.0.0 save data - migrating to v${GAME_VERSION} with clean slate`,
					);
					...
					migrationInfo.value = {
						...
						oldFactories: Object.values(state.factoryCounts || {}).reduce(
							(a, b) => a + b,
							0,
						),
				...
				setTotalQsosEarned(
					parseNonNegativeBigInt(state.totalQsosEarned || state.qsos || "0"),
				);
				const hasPrestigeLevelField = "prestigeLevel" in state;
				const hasPrestigePointsField = "prestigePoints" in state;
				const hasBothPrestigeFields =
					hasPrestigeLevelField && hasPrestigePointsField;
				...
				for (const factory of FACTORIES) {
					...
					const rawRemainder = Number(
						state.factoryProductionRemainders?.[factory.id] ?? 0,
					);
					loadedProductionTotals[factory.id] = parseNonNegativeBigInt(rawTotal);
					loadedProductionRemainders[factory.id] =
						Number.isFinite(rawRemainder) && rawRemainder >= 0
							? rawRemainder
							: 0;
				}
				...
				tapPrestigeAccumulator.value = parseNonNegativeBigInt(
					state.tapPrestigeAccumulator,
				);
				...
				if (state.lotteryState) {
					const now = Date.now();
					const savedSolarStormEndTime = Number(
						state.lotteryState.solarStormEndTime,
					);
					const normalizedSolarStormEndTime = Number.isFinite(
						savedSolarStormEndTime,
					)
						? savedSolarStormEndTime
						: 0;
					const savedIsSolarStorm = state.lotteryState.isSolarStorm === true;
					const hadActiveStormAtLoad =
						savedIsSolarStorm && now < normalizedSolarStormEndTime;

					lotteryState.value = {
						lastTriggerTime: state.lotteryState.lastTriggerTime || 0,
						isBonusAvailable:
							state.lotteryState.isBonusAvailable &&
							now < state.lotteryState.bonusAvailableEndTime,
						bonusFactoryId: state.lotteryState.bonusFactoryId || null,
						bonusEndTime: state.lotteryState.bonusEndTime || 0,
						bonusAvailableEndTime:
							state.lotteryState.bonusAvailableEndTime || 0,
						phenomenonTitle: state.lotteryState.phenomenonTitle || "",
						isSolarStorm: hadActiveStormAtLoad,
						solarStormEndTime: hadActiveStormAtLoad
				...
				upgradePurchaseMeta.value = normalizeUpgradePurchaseMeta(
					state.upgradePurchaseMeta,
				);
				...
				if (state.morseChallengeState) {
					morseChallengeState.value = {
						...
						challengeStartTime:
							state.morseChallengeState.challengeStartTime || 0,
					...
					if (
						MORSE_CHALLENGE_TERMINAL_STATES.includes(
							morseChallengeState.value.state,
						) &&
				...
				if (state.lastSaveTime) {
					...
					if (offlineMs > 0 && offlineMs <= MAX_REASONABLE_OFFLINE_MS) {
						...
						if (offlineMinutes >= MIN_OFFLINE_MINUTES) {
							...
							const offlineQsos = calculateOfflineProgress(
								offlineHours,
								productionRate,
							);
	...
	function startMorseChallenge() {
		const char =
			MORSE_CHAR_LIST[Math.floor(Math.random() * MORSE_CHAR_LIST.length)];
	...
	function handleMorseKeyTap(type) {
		...
		if (
			keyed.length === pattern.length &&
			keyed.every((v, i) => v === pattern[i])
		) {
			grantMorseBonus();
			return;
		}
