...
function handleBuyUpgrade(upgrade) {
  if (canAffordUpgrade(upgrade)) {
    const success = store.buyUpgrade(upgrade.id)
    if (success) {
      store.save()
    }
  }
        ...
        <div class="flex justify-between items-start">
          <div class="flex-1">
            ...
            <div class="flex items-center gap-4 text-sm">
              ...
              <span class="text-terminal-amber"> Effect: {{ upgrade.multiplier }}× output </span>
              <span class="text-gray-500"> Requires: {{ upgrade.threshold }} owned </span>
            </div>
          </div>
