...
<template>
  <div class="error-boundary">
    ...
    <div v-else class="min-h-screen bg-terminal-bg p-8 flex items-center justify-center">
      <div class="max-w-2xl w-full border-2 border-red-600 bg-gray-900 p-8 rounded">
        <div class="text-center mb-6">
          <span class="text-6xl">💥</span>
        </div>

        <h1 class="text-2xl font-bold text-red-500 text-center mb-4">Something went wrong!</h1>

        <div class="bg-gray-800 p-4 rounded mb-6 font-mono text-sm">
          <p class="text-red-400 mb-2">
            <strong>Error:</strong> {{ error?.message || 'Unknown error' }}
          </p>
          <p v-if="errorInfo" class="text-gray-400"><strong>Location:</strong> {{ errorInfo }}</p>
        </div>
        ...
        <p class="text-xs text-gray-500 mt-6 text-center">
          If this error persists, please export your save data from the Settings tab before
          reloading.
        </p>
      </div>
    </div>
