...
const shouldTimerRun = computed(() => {
  ...
  const hasActiveStorm =
    store.lotteryState.isSolarStorm && now.value < store.lotteryState.solarStormEndTime
  ...
  <div class="space-y-2">
    ...
    >
      <div class="flex items-center justify-between">
        <div class="flex-1">
          <p class="text-terminal-amber font-bold mb-1">{{ phenomenonTitle.toUpperCase() }}!</p>
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <div>
            ...
            <p class="text-sm text-gray-400">
              Output increased from {{ formatRate(baseOutput) }}/sec to
              {{ formatRate(boostedOutput) }}/sec ({{ boostedFactoryCount }} ×
              {{ boostedFactory.qsosPerSecond }}/sec)
            </p>
          </div>
        </div>
