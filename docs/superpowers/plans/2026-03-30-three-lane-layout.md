...
- Repurpose the existing "Signal Grid" (`FactoryCard.vue` and `FactoryList.vue`) to serve purely as the visual representation of *purchased* factories in the center lane.
...
Currently, `FactoryCard` has a "Buy" button. For the Center Lane, we only want to show *purchased* factories, and we don't necessarily need a buy button anymore (or we can hide it via a prop `readOnly`).
